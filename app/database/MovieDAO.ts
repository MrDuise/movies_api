...
export class MovieDAO
{
    ...
    public findAllMovies(callback: any)
    {
        ...
        this.pool.getConnection(async function(err:any, connection:any)
        {
            ...
            let result1 = await connection.query('SELECT * FROM Movies ORDER BY QUANTITY');
            for(let x=0;x < result1.length;++x)
            {
                // Add Movie to the list
                movies.push(new Movie(result1[x].id, result1[x].title, result1[x].genre, result1[x].price, result1[x].quantity)); 
            }
    ...
    public findSingleMovie(movieID: number, callback: any)
    {
        ...
        this.pool.getConnection(async function(err:any, connection:any)
        {
            ...
            // Use Promisfy Util to make an async function and run query to get all Movies
            connection.query = util.promisify(connection.query);
            let result1 = await connection.query('SELECT * FROM Movies WHERE id =?', [movieID]);
            for(let x=0;x < result1.length;++x)
            {
                ...
                // Add Movie to the list
                movie = new Movie(result1[x].id, result1[x].title, result1[x].genre, result1[x].price, result1[x].quantity); 
            }
    ...
    public create(movie:Movie, callback: any)
    {
        ...
        this.pool.getConnection(async function(err:any, connection:any)
        {
            ...
            connection.query = util.promisify(connection.query);
            let result1 = await connection.query('INSERT INTO MOVIES (TITLE, GENRE, PRICE, QUANTITY) VALUES(?,?,?,?)', [movie.Title, movie.Genre, movie.Price, movie.Quantity]);
    ...
    public update(movie:Movie, callback: any)
    {
         ...
         this.pool.getConnection(async function(err:any, connection:any)
         {
            ...
            if (err) throw err;
             ...
             let changes = 0;
             connection.query = util.promisify(connection.query);
            let result1 = await connection.query('UPDATE MOVIES SET TITLE=?, GENRE=?, PRICE=?, QUANTITY=? WHERE ID=?', [movie.Title, movie.Genre, movie.Price, movie.Quantity, movie.Id]);
